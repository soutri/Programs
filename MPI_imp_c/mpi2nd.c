...
{
int myid,j,*data,tosum[25],sums[8] ;
MPI_Init(&argc ,&argv ) ;
MPI_Comm_rank(MPI_COMM_WORLD,&myid);
if(myid==0) /* Manager allocates and initialises data*/
{
...
if(v>0) /* After the scatter everynode has 25 numbers to sum */
{
...
for(j =0;j<25;j++) 
	sums[myid ] += tosum[j];
if(v>0) 
	printf( "Node %d computes the sum %dnn" ,myid,sums[myid]);
